...
Proof.
  ...
  intros x y1 y2 Hy1.
  generalize dependent y2.
  induction Hy1; subst; intros y2 Hy2;
...
Qed.
  (*对value inversion*)
  (*- inversion H1; subst. inversion Hy1. (*REVIEW*)*)
(** [] *)
...
Proof.
  induction t.
  - left. constructor.
  - right. destruct IHt1 as [H1 | [t'1 H1]].
    + destruct IHt2 as [H2 | [t'2 H2]].
      * inversion H1.
        inversion H2.
        subst.
        exists (C(n + n0)).
...
Proof.
  intros v. unfold normal_form.
  intros H1 H2. inversion H1. subst.
  inversion H2. (*destruct作用*)
  inversion H.
Qed.
(*
...
Proof. (* a corollary of [strong_progress]... *)
  ...
  specialize strong_progress with (t:=t) as G.
  (*assert (G : value t \/ exists t', t --> t').
  { apply strong_progress. }*)
  destruct G as [G | G].
  - (* l *) apply G.
  - (* r *) apply H in G. inversion G.
  (*exfalso. apply H. apply G.*)
  (*contradiction.*)
Qed.
...
Proof.
  ...
  induction P11; inversion P21; subst; clear P21.
  - reflexivity.
  - exfalso. apply P12. exists y. assumption.
    (*这里应该是P12与H矛盾了，但是可以直接exfalso提出false*)
  - exfalso. apply P22. exists y. assumption.
  - apply IHP11.
    + assumption.
    + remember (step_deterministic x y y0) as OK.
      rewrite (OK H H0).
      assumption.
Qed.
(** [] *)
...
Proof.
  ...
  - (* P *)
    ...
    destruct Hnormal1 as [n1]. (*REVIEW*)
...
Proof.
  ...
  induction H.
  - apply multi_refl.
  - eapply multi_trans. {apply multistep_congr_1.
                          apply IHeval1. }
    eapply multi_trans. {apply multistep_congr_2.
                          - apply v_const.
                          - apply IHeval2. }
    apply multi_R.
    apply ST_PlusConstConst.
...
Proof.
  ...
  intros [H1 H3].
  assert (exists n, t'= C n) as H2. { rewrite nf_same_as_value in H3.
