...
(** **** Exercise: 1 star, standard (subtype_concepts_tf)
    ...
    - There exists a type that is a subtype of every other type.
      false   Base_Type is already the lowest
    ...
    - There exists an arrow type that is a supertype of every other
      arrow type.
      false   since A->B <: T -> Top , there must be a T <: A (forall A)
    - There exists an arrow type that is a subtype of every other
      arrow type.
      flase   since Top->T <: A->B , there must be a T <: B (forall B)
    - There is an infinite descending chain of distinct types in the
      ...
      false   all type component will be reduced to a base type
    - There is an infinite _ascending_ chain of distinct types in
      ...
      true    record can be expanded infinitely
*)
...
Proof with eauto.
  ...
  - intros.
    apply IHHs2 in HeqV.
    destruct HeqV as [U3 [U4 [H1 H2]]].
    apply IHHs1 in H1.
    destruct H1 as [U5 [U6 [H3 H4]]].
    exists U5.
    exists U6.
    split; [assumption | ].
    destruct H4.
...
(** **** Exercise: 2 stars, standard (variations)
    ...
    - Suppose we add the following typing rule:
                    ...
                    -----------------------------------    (T_Funny1)
                          ...
                          nothing happens, because we dont add new rules in fact
    - Suppose we add the following reduction rule:

                             --------------------         (ST_Funny21)
                             unit --> (\x:Top. x)

                          preservation : unit can be typing as Unit 
                                        while  (\x:Top. x) can't be typing as Unit
    - Suppose we add the following subtyping rule:

                               ----------------          (S_Funny3)
                               Unit <: Top->Top

                          progress : type of (unit x) can be reduced to Top
                                    but this term can't be reduced 
                                          and is not a value
    - Suppose we add the following subtyping rule:
                          ...
                          suppose f : Top->Top, g: Unit->Top
                          (g f) cant be reduced
                                but can be typing as Top
                          so we dont have progress

                          preservation is true : we can't reduce a abs
    ...
    - Suppose we add the following reduction rule:
                          ...
                          nothing happens since (unit t) cant be typing
    - Suppose we add the same reduction rule _and_ a new typing rule:
              ...
              (unit t) will be typing as Top
              (t unit) will be either typing as sth 
                              or nothing(if t isn't a proparate abs)
              so preservation goes false

              we only get new type rule when (unit t) but it can be typing
    ...
    - Suppose we _change_ the arrow subtyping rule to:
                      ...
                      thing goes wrong when app 
                      suppose x : T1 t : S1->U
                      t can be typing as (S1->U) as (T1->U)
                      so (t x) can be typing as (U)
                      but we know that (t x) can't be reduced
                      (since x : T1 <: S1 , t may use components in (S1 - T1))
                      therefore, progress is false

                      considering preservation, nothing happends, because we can't
                      reduce term more than before
*)    
