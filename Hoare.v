...
(** This example also illustrates a convention that we'll use
    ...
    capital let ters like [X], [Y], and [Z] are Imp variables, while
...
(* 
（1） （2） （3） 
（4）：原因前条件是False， False -> all，自然有后条件任意
（6）：同样False -> True
（7）：不会停止的循环
（8）
    [] *)
...
Qed.
(*
  eapply hoare_consequence; [ | | try (unfold "->>", assertion_sub, t_update; simpl; intros st H; apply H)].
  - apply hoare_asgn.
  - assertion_auto.
Qed.*)
...
Proof.
  ...
  - eapply hoare_consequence_pre.
    ...
    + assertion_auto''.
    (*
      unfold "->>", assertion_sub, bassertion, t_update.
      simpl.
      intros st [_ H].
      rewrite leb_le in H.
      Search (_ + (_ - _)).
      rewrite Arith_prebase.le_plus_minus_r_stt; auto.
    *)
  - eapply hoare_consequence_pre.
    + apply hoare_asgn.
    + assertion_auto''.
...
Theorem always_loop_hoare : forall Q, (*REVIEW*)
