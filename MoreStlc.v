...
Fixpoint subst (x : string) (s : tm) (t : tm) : tm :=
  ...
  (* let *)
  | <{let xx = t1 in t2}> =>  (*WHY???*)
      <{ {if String.eqb x xx then <{let xx = [x:=s]t1 in t2}> else <{let xx = [x:=s]t1 in [x:=s]t2}>} }>
  (* fix *)
  | <{fix t}> => <{fix [x := s] t}>
...
Proof with eauto.
  ...
  - (*t.fst*)
    right.
    destruct IHHt...
    + destruct H; try solve_by_invert... (*REVIEW：获得v的结构*)
    + destruct H as [t' H]...
  - (*t.snd*)
    right.
    destruct IHHt...
  ...
  - (* let *)
    right.
    destruct IHHt1... (*x=v1显然有[x:=v1]t2*)
    destruct H as [t1' H]... (*t1 -> t1'*)
  - (* fix *)
    right.
    destruct IHHt...
    + destruct H; try solve_by_invert. (*获取t的函数结构*)
      (*eauto秒了但是复习Y combinator*)
      exists <{ [x0 := {tm_fix <{\x0:T2,t1}> } ] t1 }> .
      apply ST_FixAbs.
    + destruct H as [t' H]...
Qed.
(** [] *)
...
Proof with eauto.
  ...
  - (*tm_let*)  
    rename s into xx.
    destruct (eqb_spec x xx); subst; eapply T_Let...
    + (*x = xx*)
